...
class Domain:
    ...
    def __init__(self, x0, xf, y0, yf, N, dirstr=None, namestr=None):
        self.x0 = x0                                    # left limit x = x0
        self.xf = xf                                    # right limit x = xf 
        self.y0 = y0                                    # lower limit y = y0
        self.yf = yf                                    # upper limit y = max h
        
        self.N = N                                      # grid scale: number of points in [0,1]
        self.Nx = int((xf-x0)*N + 1)                    # total number of x grid points
        self.Ny = int((yf-y0)*N + 1)                    # total number of y grid points
        
        self.dx = 1/N                                   # grid spacing in x
        self.dy = 1/N                                   # grid spacing in y
        
        self.xs = np.linspace(x0, xf, self.Nx)          # x grid
        self.ys = np.linspace(y0, yf, self.Ny)          # y grid
...
class Height(Domain):
    ...
    def __init__(self, x0, xf, y0, yf, N, hs, i_peaks):
...
class Space(Domain):
    def __init__(self, x0, xf, y0, yf, N, BC, Re, namestr):
        
        super().__init__(x0, xf, y0, yf, N)
        
        dirstr = f"./stokes_examples/{namestr}"  
        self.dirstr = dirstr                            # directory to write to
        self.filestr = f"{dirstr}/{namestr}_N{N}"       # file string to write to
...
def center_second_diff(fs, Nx, dx):
    ...
    fs_dxx[0]= right_second(dx, fs[0 : 4])
    fs_dxx[Nx-1]= left_second(dx, fs[Nx-4 : Nx]) 
...
def center_third_diff(fs, Nx, dx):
    ...
    fs_dxxx[0]= right_third(dx, fs[0 : 5]) 
    fs_dxxx[1]= right_third(dx, fs[1 : 6])
    fs_dxxx[Nx-1]= left_third(dx, fs[Nx-5 : Nx])
    fs_dxxx[Nx-2]= left_third(dx, fs[Nx-6 : Nx-1]) 
...
def center_fourth_diff(fs, Nx, dx):
    ...
    fs_dxxxx[0]= right_fourth(dx, fs[0 : 6])
    fs_dxxxx[1]= right_fourth(dx, fs[1 : 7])
    fs_dxxxx[Nx-1]= left_fourth(dx, fs[Nx-6 : Nx])
    fs_dxxxx[Nx-2]= left_fourth(dx, fs[Nx-7 : Nx-1]) 
